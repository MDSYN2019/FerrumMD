...
use crate::molecule::molecule::{
    apply_all_bonded_forces_and_energy, apply_bonded_forces_and_energy, Angle, Dihedral, Improper,
};
...
impl SystemVerletNeighborList {
    ...
    fn rebuild_pairs(&mut self, systems: &[System], box_length: f64) {
        ...
        let mut atom_map: Vec<(usize, usize)> = Vec::with_capacity(self.reference_positions.len());
...
fn compute_intermolecular_forces_systems_with_pairs(
    ...
    total_energy += compute_intramolecular_pair_lj_forces_and_energy(systems, box_length, cutoff);
...
fn intramolecular_pair_lj_energy_systems(systems: &[System], box_length: f64, cutoff: f64) -> f64 {
    let cutoff2 = cutoff * cutoff;
    let mut energy = 0.0;
    for sys in systems {
        for (&(i, j), scaling) in &sys.pair_scalings {
            ...
            let r = safe_norm(r2.sqrt());
            let sigma = 0.5 * (sys.atoms[i].lj_parameters.sigma + sys.atoms[j].lj_parameters.sigma);
            let epsilon =
                (sys.atoms[i].lj_parameters.epsilon * sys.atoms[j].lj_parameters.epsilon).sqrt();
            energy += scaling.lj_scale * lennard_jones_potential(r, sigma, epsilon);
        }
    }
    energy
...
pub fn apply_thermostat_berendsen(
    ...
    match state {
        ...
        InitOutput::Systems(systems) => {
            ...
            for sys in systems.iter_mut() {
                apply_thermostat_berendsen_particles(&mut sys.atoms, target_temperature, tau, dt);
            }
...
fn add_electrostatic_real_space_particles(
    ...
    for i in 0..particles.len() {
        for j in (i + 1)..particles.len() {
            ...
            let rij =
                minimum_image_convention(particles[j].position - particles[i].position, box_length);
            let r = rij.norm();
            if r <= 1e-12 || r > rc {
                continue;
            }
            ...
            let scalar = qq
                * (erfc_ar / (r * r) + (2.0 * alpha / std::f64::consts::PI.sqrt()) * exp_term / r);
...
fn add_electrostatic_reciprocal_systems(
    ...
    let self_energy: f64 =
        charges.iter().map(|q| q * q).sum::<f64>() * (-k_e * alpha / std::f64::consts::PI.sqrt());
    energy += self_energy;
...
pub fn run_md_nve_particles(
    ...
    let electrostatic_init_energy = add_electrostatic_forces_particles(particles, box_length, &pme);
    ...
    for _step in 0..number_of_steps {
        ...
        let electrostatic_energy = add_electrostatic_forces_particles(particles, box_length, &pme);
        ...
        if thermostat == "berendsen" {
            ...
            apply_thermostat_nose_hoover_particles(particles, 300.0, 10.0, dt, &mut xi_nose_hoover)
        }
